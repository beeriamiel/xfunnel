...
export function Verticals({ companyId, accountId, selectedRegion, onSelectVertical, onBack }: VerticalsProps) {
  ...
  const { data, error, isLoading } = useSWR(
    companyId && selectedRegion ? `vertical-analysis-${companyId}-${selectedRegion}-${timePeriod}-${selectedProductId}` : null,
    () => companyId && selectedRegion ? getAnalysisByVertical(companyId, accountId, selectedRegion, timePeriod, isSuperAdmin, selectedProductId) : null
  )
              ...
              <Card className="p-4 hover:bg-accent/50 transition-colors">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="font-medium">{vertical.vertical}</h3>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="text-muted-foreground hover:text-foreground"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Compare performance across different verticals
                </p>
              </div>
              <div className="flex items-center gap-2">
                {METRICS.map((metric) => (
