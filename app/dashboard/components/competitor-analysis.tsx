  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // Calculate percentages and process mentions data for each engine
        Object.entries(engineData).forEach(([currentEngine, data]) => {
          ...
          data.mentions.forEach(item => {
            item.mentions.percentage = totalMentions > 0 ? (item.mentions.count / totalMentions) * 100 : 0;
          });

          // Get the main company's data
          ...
          // Process mentions: get top 5 + rest
          ...
          // Calculate "Rest" for mentions
          const restMentionedCompanies = otherCompanies.filter(company => !topMentionedCompanies.includes(company));
          ...
          // Create the final mentions data array
          const mentionsData = [mainCompanyData, ...topMentionedCompanies];

          // Add Rest category if there are remaining companies
          ...
          // Update the engine data with processed mentions
          engineData[currentEngine].mentions = mentionsData;
        });

        setCompetitorData(engineData);
      } catch (err) {
