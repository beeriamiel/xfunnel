...
function CompanySetup({ onComplete }: { onComplete: (completedICPs: ICP[], completedPersonas: Persona[]) => void }) {
  const [step, setStep] = useState<'initial' | 'product' | 'competitors' | 'icps' | 'personas' | 'complete'>('initial')
  ...
  const [editingStep, setEditingStep] = useState<'initial' | 'product' | 'competitors' | 'icps' | 'personas' | null>(null)

  const handleEditStep = (type: 'initial' | 'product' | 'competitors' | 'icps' | 'personas') => {
    setEditingStep(type)
    setStep(type)
