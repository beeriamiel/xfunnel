                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <Badge variant="outline" className="text-xs border-[#f9a8c9]">
                            <span>{query.icp.region} · {query.icp.vertical}</span>
                          </Badge>
                          <Badge variant="outline" className="text-xs border-[#f9a8c9]">
...
function CompanySetup({ onComplete, onTransitionStart }: CompanySetupProps) {
  // Remove duplicate state that should be in parent
  ...
  // Event handler types
  const handleCompanyNameChange = (e: ChangeEvent<HTMLInputElement>) => setCompanyName(e.target.value)

  const handleQueryChange = (e: ChangeEvent<HTMLInputElement>) => setNewQuery(e.target.value)

  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>, combinationId: string) => {
    if (e.key === 'Enter' && newQuery.trim()) {
      handleAddQuery(combinationId)
    }
  }

  // Map callback types
  const mapCompetitor = (c: Competitor) => c.id !== id
  const mapProduct = (p: Product) => p.id !== id
  const mapICP = (i: ICP) => i.id !== id
  const mapPersona = (p: Persona) => p.id !== id
                      ...
                      <div className="py-4 px-6 bg-[#f6efff]/50 space-y-4">
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            onKeyDown={(e) => handleKeyDown(e, combination.id)}
                          />
                          <Button
...
export function ResponseAnalysis({ companyId }: Props) {
  ...
  const handleTransitionStart = () => {
    ...
    // Additional confetti bursts during the animation
    setTimeout(() => {
      confetti({
        particleCount: 30,
        spread: 60,
