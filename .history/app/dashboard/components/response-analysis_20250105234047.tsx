...
function generateQueries(icp: ICP, persona: Persona): ICPPersonaCombination['queries'] {
  return [
    {
      id: crypto.randomUUID(),
  ...
  return [
    {
      ...
      icp: {
        id: crypto.randomUUID(),
        name: vertical,
        // Add other required ICP properties
      },
      persona: {
        id: crypto.randomUUID(),
        title,
...
function ResponseTable({ 
  ...
  const handleGenerateQueries = async (combination: ICPPersonaCombination) => {
    setCombinations((prev: ICPPersonaCombination[]) => prev.map((c) => 
      c.id === combination.id 
        ? { 
            ...c, 
            queries: generateMockQueries(c.vertical, c.title),
            isGeneratingQueries: false,
            isExpanded: true,
            hasAnalysis: false,
            lastAnalysisRun: null,
                        ...
                        <ScrollArea className="h-[200px] rounded-md border border-[#f9a8c9]/20 bg-white">
                          <div className="p-4 space-y-3">
                            {combination.queries.map((query: Query) => {
                              const hasIcpInfo = query.icp?.name
                              const hasPersonaInfo = query.persona?.title
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex gap-2 mt-2">
                                    {hasIcpInfo && query.icp && (
                                      <Badge variant="outline">
                                        {query.icp.name}
                                      </Badge>
                                    )}
                                    {hasPersonaInfo && query.persona && (
                                      <Badge variant="outline">
                                        {query.persona.title}
                                      </Badge>
                                    )}
