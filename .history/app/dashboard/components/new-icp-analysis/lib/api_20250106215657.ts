...
// Helper functions for stage filtering and metrics
function filterValidMentionStages(
  data: Database['public']['Tables']['response_analysis']['Row'][]
...
function filterValidRankingStages(
  data: Database['public']['Tables']['response_analysis']['Row'][]
): Database['public']['Tables']['response_analysis']['Row'][] {
  const validStages = ['solution_comparison', 'final_research'];
  return data.filter(item => 
...
function filterValidFeatureStages(
  data: Database['public']['Tables']['response_analysis']['Row'][]
): Database['public']['Tables']['response_analysis']['Row'][] {
  return data.filter(item => 
...
// Update the main metric calculation functions
function calculateMentionedRate(
  data: Database['public']['Tables']['response_analysis']['Row'][]
): number {
  const validData = filterValidMentionStages(data);
  if (!validData.length) return 0;

  const mentions = validData.map(item => {
    let weight = 1.0;
    let isMentioned = false;

    // Check both direct mention and mentioned_companies array
    ...
    if (isMentioned) {
      ...
      // Weight multiple mentions higher
      if (item.mentioned_companies && item.mentioned_companies.length > 0) {
        const companyName = item.company_name.toLowerCase();
        ...
        if (mentionCount > 1) {
          weight *= Math.min(1.3, 1 + (mentionCount * 0.1));
        }
      }
    }

    return { value: isMentioned ? 100 : 0, weight };
  });

  return Math.round(calculateWeightedMetric(mentions));
}
