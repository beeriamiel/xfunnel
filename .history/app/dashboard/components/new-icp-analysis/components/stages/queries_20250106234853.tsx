      ...
      <div className="space-y-4">
        {PHASE_ORDER.map((phase, index) => {
          ...
          return (
            ...
            >
              <Card className="group overflow-hidden border-primary/20 bg-primary/5 hover:bg-primary/10 hover:shadow-[0_0_15px_rgba(124,58,237,0.1)] transition-all duration-300">
                <button
                  onClick={() => togglePhase(phase)}
                  className="w-full flex items-center gap-4 p-4 text-left hover:bg-primary/5 transition-all duration-300"
                >
                  <div className="relative flex items-center">
                    <motion.div 
                      className={cn(
                        "h-8 w-8 rounded-full flex items-center justify-center text-sm font-medium transition-all duration-300 shadow-sm",
                        expandedPhase === phase
                          ? "bg-primary text-primary-foreground shadow-[0_0_10px_rgba(124,58,237,0.2)]"
                          : "bg-primary/10 text-primary hover:bg-primary/20 hover:shadow-[0_0_10px_rgba(124,58,237,0.1)]"
                      )}
                    ...
                    {index < PHASE_ORDER.length - 1 && (
                      <motion.div 
                        className="absolute left-1/2 top-8 h-12 w-[2px] bg-gradient-to-b from-primary/20 via-primary/10 to-transparent"
                        initial={{ scaleY: 0 }}
                        animate={{ scaleY: 1 }}
...
function QueryCount({ count, total }: QueryCountProps) {
  return (
    <div className="flex items-center gap-4">
      <div className="px-2.5 py-1 bg-primary/5 text-primary border border-primary/20 rounded-full flex items-center gap-1.5">
        <span className="text-xs font-medium">{count}</span>
        <span className="text-xs text-primary/60">/</span>
        <span className="text-xs text-primary/60">{total}</span>
        <span className="text-xs text-primary/60">responses</span>
      </div>
