  ...
  return (
    ...
    >
      <div className="flex items-center justify-end mb-6">
        ...
        >
          <Badge variant="secondary" className="text-sm bg-gradient-to-r from-purple-600/20 to-purple-500/10 border border-purple-600/20">
      ...
      <div className="space-y-4">
        {PHASE_ORDER.map((phase, index) => {
          ...
          return (
            ...
            >
              <Card className="group overflow-hidden border border-purple-600/20 bg-gradient-to-r from-purple-950/50 to-purple-900/30 hover:shadow-lg transition-all duration-300">
                <button
                  onClick={() => togglePhase(phase)}
                  className="w-full flex items-center gap-4 p-3 text-left hover:bg-purple-900/20 transition-all duration-300"
                >
                  <div className="relative flex items-center">
                    <motion.div 
                      className={cn(
                        ...
                        expandedPhase === phase
                          ? "bg-gradient-to-r from-purple-600 to-purple-500 text-primary-foreground shadow-lg"
                          : "bg-gradient-to-r from-purple-600/20 to-purple-500/10 text-primary hover:scale-110"
                      )}
                    ...
                    {index < PHASE_ORDER.length - 1 && (
                      <motion.div 
                        className="absolute left-1/2 top-8 h-8 w-[2px] bg-gradient-to-b from-purple-600/20 to-transparent"
                        initial={{ scaleY: 0 }}
                        animate={{ scaleY: 1 }}
...
function QueryCount({ count, total }: QueryCountProps) {
  return (
    <div className="flex items-center gap-4">
      <div className="px-2 py-0.5 bg-gradient-to-r from-purple-600/20 to-purple-500/10 rounded-full flex items-center gap-1.5 border border-purple-600/20">
        <span className="text-xs font-medium">{count}</span>
        <span className="text-xs text-muted-foreground">/</span>
        <span className="text-xs text-muted-foreground">{total}</span>
        <span className="text-xs text-muted-foreground">responses</span>
      </div>
      <div className="h-1.5 w-32 bg-purple-900/20 rounded-full overflow-hidden">
        <motion.div 
