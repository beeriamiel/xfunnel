    ...
    const loadCompanyData = async () => {
      ...
      try {
        const [
          ...
          supabase.from('competitors').select('*').eq('company_id', effectiveCompanyId),
          supabase.from('ideal_customer_profiles').select('*').eq('company_id', effectiveCompanyId),
        ...
        if (company) {
          setCompanyProfile({
            ...
            markets: company.markets_operating_in || [],
          ...
          // Map ICPs and their personas
          const icpsWithPersonas = icps?.map(icp => {
            const icpPersonas = personas?.filter(p => p.icp_id === icp.id) || [];
            return {
              id: icp.id,
              vertical: icp.vertical,
    ...
    <Card className="p-6">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Generate AI responses based on your company profile</p>
        </div>
            ...
            <>
              <CompanyProfileCard
                ...
                stats={{
                  ...
                  competitorsCount: companyProfile.competitors.length
                }}
