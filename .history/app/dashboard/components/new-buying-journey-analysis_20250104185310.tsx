...
function PersonaCard({ 
  ...
  return (
    ...
    >
      <div className="space-y-3">
        <div className={cardStyles.header}>
          <div className={`${cardStyles.iconWrapper} bg-pink-50`}>
            <User className="h-5 w-5 text-pink-500" />
          </div>
          <div className="min-w-0 flex-1">
            <h3 className={cardStyles.title}>{persona.buyer_persona}</h3>
            <p className={cardStyles.subtitle}>Buyer Persona</p>
          </div>
        </div>

        <div className={cardStyles.progress}>
          <QueryCount 
            count={persona.total_queries || 0} 
...
function QueriesSection({ 
  ...
  return (
    <div className="mt-8 space-y-6">
      <BreadcrumbPath region={region} vertical={vertical} persona={persona} />
...
function QueryCard({ 
  ...
  return (
    <Card className="p-4">
      <div className="space-y-2">
        <h3 className="text-sm font-medium text-muted-foreground">{query.question}</h3>
  ...
  useEffect(() => {
    ...
    async function fetchPersonaData() {
      try {
        ...
        const processedData = (responseData || []).reduce((acc: { [key: string]: any }, curr) => {
          const persona = curr.buyer_persona || 'unknown';
          if (!acc[persona]) {
            acc[persona] = {
          ...
          if (EARLY_PHASES.includes(phase)) {
          ...
          if (queryInfo && curr.query_id) {
            ...
            if (!acc[persona].queries.has(queryKey)) {
              acc[persona].queries.set(queryKey, {
                ...
                buyerJourneyPhase: curr.buying_journey_stage || 'unknown',
                engineResults: {},
            ...
            if (query && curr.answer_engine) {
              ...
              if (frontendEngineKey) {
                ...
                query.engineResults[frontendEngineKey] = {
                  ...
                  citations: curr.citations_parsed && typeof curr.citations_parsed === 'object' && 'urls' in curr.citations_parsed
                    ? curr.citations_parsed.urls
                    : [],
    ...
    fetchPersonaData();
  }, [companyId, vertical, region, timeSegment]);
