...
import React, { useState, useRef, useEffect, ChangeEvent, KeyboardEvent, MouseEvent, FormEvent } from 'react'
...
import type {
  ...
  StepId,
  StepIndicatorProps,
  CompanySetupProps as BaseCompanySetupProps,
  ICPPersonaCombination,
  SelectableCardProps as BaseSelectableCardProps
} from './types'
...
interface CustomSelectableCardProps extends Omit<BaseSelectableCardProps, 'icon'> {
  icon: React.ElementType;
}

interface CompanySetupProps extends BaseCompanySetupProps {
  setShowSuccessAnimation: (val: boolean) => void;
  setIsTransitioning: (val: boolean) => void;
  setHasCompletedOnboarding: (val: boolean) => void;
  setCompanyProfile: (profile: { icps: ICP[]; personas: Persona[]; products: any[]; competitors: any[] }) => void;
}
...
function generateMockQueries(vertical: string, title: string): ICPPersonaCombination['queries'][number][] {
  return [
    {
...
function CompanySetup({ 
  ...
  const handleUpdateCompetitor = () => {
    ...
    setCompetitors(competitors.map((c: Competitor) => 
  ...
  const handleEditStep = (type: CompletedStep['type']) => {
    ...
    const index = completedSteps.findIndex((s: CompletedStep) => s.type === type)
  ...
  const handleDeleteICP = (id: number) => {
    setICPs(icps.filter((i: ICP) => i.id !== id))
  }
  ...
  const handleUpdatePersona = () => {
    ...
    setPersonas(personas.map((p: Persona) => 
      p.id === editingPersona.id 
        ? { ...editingPersona, ...newPersona as Persona }
        : p
    ))
  ...
  const handleDeletePersona = (id: number) => {
    setPersonas(personas.filter((p: Persona) => p.id !== id))
  }
                          ...
                          {[0, 1, 2, 3].map((index) => (
                            <div key={index} className="flex items-center gap-2">
                              ...
                              <Input
                                ...
                                onChange={(e: ChangeEvent<HTMLInputElement>) => handleCompetitorNameChange(index, e.target.value)}
                              />
                            </div>
                          ))}
                        </div>
            ...
            {step === 'personas' && !isLoading && (
              <Card className="w-full max-w-4xl p-6">
                <div className="space-y-6">
                  ...
                  <div className="flex justify-between items-center pt-4">
                    <Dialog open={dialogOpen.persona} onOpenChange={(open: boolean) => setDialogOpen({ ...dialogOpen, persona: open })}>
                      <DialogTrigger asChild>
                        <Button className="bg-[#30035e] hover:bg-[#30035e]/90">
                        ...
                        <div className="grid gap-4 py-4">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e: ChangeEvent<HTMLInputElement>) => setNewPersona({ ...newPersona, title: e.target.value })}
                            />
                          </div>
                          <div className="space-y-2">
...
function ResponseTable({ 
  ...
  const [filteredCombinations, setFilteredCombinations] = useState<ICPPersonaCombination[]>([])
  ...
  const handleGenerateQueries = async (combination: ICPPersonaCombination) => {
    setCombinations((prev: ICPPersonaCombination[]) => prev.map((c: ICPPersonaCombination) => 
      c.id === combination.id 
        ? { 
            ...c, 
  ...
  const handleQueryChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (query.length > 0) {
      const filtered = combinations.filter((c: ICPPersonaCombination) => 
                      ...
                      <HoverCardContent className="w-auto">
                        <div className="space-y-2">
                          ...
                          {combination.analysisHistory?.length > 0 ? (
                            <div className="space-y-1">
                              {combination.analysisHistory.slice().reverse().map((run: AnalysisRun, index: number) => (
                      ...
                      <div className="py-4 px-6 bg-[#f6efff]/50 space-y-4">
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            value={newQuery}
                            onChange={(e: ChangeEvent<HTMLInputElement>) => setNewQuery(e.target.value)}
