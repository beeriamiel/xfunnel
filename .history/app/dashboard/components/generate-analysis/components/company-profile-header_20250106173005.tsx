...
export function CompanyProfileHeader({ companyId }: CompanyProfileHeaderProps) {
  ...
  return (
    <Card className="p-6">
      ...
      {/* Stats Row */}
      <div className="flex flex-wrap gap-x-12 mb-8">
        <div className="flex items-center">
          <Package className="h-3.5 w-3.5 text-purple-700 mr-1" />
          <span className="text-sm font-medium mr-1">{companyData.main_products.length}</span>
          <span className="text-sm text-gray-500">Products</span>
        </div>
        <div className="flex items-center">
          <Globe2 className="h-3.5 w-3.5 text-purple-700 mr-1" />
          <span className="text-sm font-medium mr-1">{companyData.markets_operating_in.length}</span>
          <span className="text-sm text-gray-500">Markets</span>
        </div>
        <div className="flex items-center">
          <Building2 className="h-3.5 w-3.5 text-purple-700 mr-1" />
          <span className="text-sm font-medium mr-1">{companyProfile?.icps?.length || 0}</span>
          <span className="text-sm text-gray-500">ICPs</span>
        </div>
        <div className="flex items-center">
          <Users className="h-3.5 w-3.5 text-purple-700 mr-1" />
          <span className="text-sm font-medium mr-1">{companyProfile?.personas?.length || 0}</span>
