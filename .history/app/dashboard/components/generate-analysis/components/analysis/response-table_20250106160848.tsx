...
function getLatestResponseInfo(queries: Query[]) {
  ...
  // TODO: This should be replaced with actual response data from the database
  // We need to fetch responses where query_id is in queryIds
  // For now, we'll use the data we have, but this should be updated
  const responses = queries
    .flatMap(query => query.responses || [])
    .filter(response => response.query_id && queryIds.includes(response.query_id));
