...
export function ResponseTable({ 
  ...
  useEffect(() => {
    ...
    icps.forEach(icp => {
      if (!icp.id) return // Skip if no ICP ID
      
      // Use only the personas assigned to this ICP
      icp.personas.forEach(persona => {
        if (!persona.id) return // Skip if no Persona ID
        
        const combinationId = `${String(icp.id)}-${String(persona.id)}`
  ...
  const handleAddQuery = (combinationId: string, text: string) => {
    ...
    const persona = icp?.personas.find(p => p.id === personaId)
