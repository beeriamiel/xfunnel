...
function getLatestResponseInfo(queries: Query[]) {
  ...
  // TODO: Replace this with actual API call to get responses
  // For now, assuming responses are passed in query.responses
  for (const query of queries) {
    if (query.responses?.length) {
      totalResponses++;
      const responseDate = query.responses[0].created_at;
      if (responseDate && (!latestDate || new Date(responseDate) > new Date(latestDate))) {
