...
function getLatestResponseInfo(queries: Query[]) {
  ...
  for (const query of queries) {
    ...
    if (query.responses?.length) {
      ...
      // Get the latest response date for this query
      const queryLatestDate = query.responses.reduce((latest, response) => {
        const responseDate = response.created_at;
        if (responseDate && (!latest || new Date(responseDate) > new Date(latest))) {
          return responseDate;
        }
        return latest;
      }, null as string | null);
      
      // Update the overall latest date if this query's latest date is more recent
      if (queryLatestDate && (!latestDate || new Date(queryLatestDate) > new Date(latestDate))) {
        latestDate = queryLatestDate;
      }
    }
  }
