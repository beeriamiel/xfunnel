...
function getLatestResponseInfo(queries: Query[]) {
  ...
  // First get all responses from all queries
  const allResponses = queries.flatMap(query => query.responses || []);
  totalResponses = allResponses.length;
  
  // Find the latest created_at date
  if (allResponses.length > 0) {
    latestDate = allResponses.reduce((latest, response) => {
