...
function getLatestResponseInfo(queries: Query[]) {
  console.log('Debugging queries:', queries);
  
  let latestDate: string | null = null;
  let totalResponses = 0;
  
  for (const query of queries) {
    console.log('Query responses:', query.responses);
    // Check for responses and their batch dates
    if (query.responses?.length) {
      totalResponses++;
      const responseBatchDate = query.responses[0].created_at;
      console.log('Response batch date:', responseBatchDate);
      if (responseBatchDate && (!latestDate || new Date(responseBatchDate) > new Date(latestDate))) {
        latestDate = responseBatchDate;
      }
    }
  }
  
  console.log('Final latestDate:', latestDate, 'totalResponses:', totalResponses);
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent>
          <p>Last run: {format(new Date(latestDate), 'MMM d, yyyy HH:mm')}</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
