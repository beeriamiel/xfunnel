...
export function ResponseTable({ 
  ...
  useEffect(() => {
    ...
    icps.forEach(icp => {
      if (!icp?.id || !Array.isArray(icp.personas)) return
      
      icp.personas.forEach(persona => {
        if (!persona?.id) return
        
        const icpIdStr = String(icp.id)
        const personaIdStr = String(persona.id)
        
        if (icpIdStr === 'undefined' || personaIdStr === 'undefined' || 
            icpIdStr === 'NaN' || personaIdStr === 'NaN') return
  ...
  const handleGenerateQueries = async (combination: ICPPersonaCombination) => {
    ...
    if (!icpId || !personaId || icpId === 'NaN' || personaId === 'NaN') {
      setIsGenerating(null)
      return
    }
    
    const icp = icps.find(i => i?.id && String(i.id) === icpId)
    const persona = icp?.personas?.find(p => p?.id && String(p.id) === personaId)
