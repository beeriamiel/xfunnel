...
function getLatestResponseInfo(queries: Query[]) {
  ...
  // Get responses for these query IDs from the responses table
  // This should be done via an API call or data fetching hook
  // For now, assuming we get the responses data along with the queries
  const responses = queries.flatMap(query => 
    query.responses?.filter(response => 
      response.query_id && queryIds.includes(response.query_id)
    ) || []
  );
  
  totalResponses = responses.length;
  
  // Find the latest created_at date from responses
  if (responses.length > 0) {
    latestDate = responses.reduce((latest, response) => {
      if (!response.created_at) return latest;
      if (!latest) return response.created_at;
      
      return new Date(response.created_at) > new Date(latest) 
        ? response.created_at 
        : latest;
    }, null as string | null);
  }
