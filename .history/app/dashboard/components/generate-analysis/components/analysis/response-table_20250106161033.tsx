...
function getLatestResponseInfo(queries: Query[]) {
  ...
  // Simply get all responses from all queries
  for (const query of queries) {
    if (query.responses) {
      totalResponses += query.responses.length;
      
      // Check each response's created_at date
      for (const response of query.responses) {
        if (response.created_at && (!latestDate || new Date(response.created_at) > new Date(latestDate))) {
          latestDate = response.created_at;
        }
      }
    }
  }
