...
function getLatestResponseInfo(queries: Query[]) {
  ...
  console.log('First query details:', queries[0]);
  
  for (const query of queries) {
    console.log(`Query ${query.id}:`, {
      lastResponseDate: query.lastResponseDate,
      responses: query.responses,
      created_at: query.created_at
    });
    
    if (query.lastResponseDate) {
      totalResponses++;
      if (!latestDate || new Date(query.lastResponseDate) > new Date(latestDate)) {
        latestDate = query.lastResponseDate;
      }
    }
  }
  
  console.log('Latest date found:', latestDate, 'Total responses:', totalResponses);
