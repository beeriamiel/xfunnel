...
export function ResponseTable({ 
  ...
  const handleGenerateQueries = async (combination: ICPPersonaCombination) => {
    ...
    const icp = icps.find(i => i.id === combination.id.split('-')[0])
    const persona = personas.find(p => p.id === combination.id.split('-')[1])
                ...
                {expandedId === combination.id && (
                  <TableRow>
                    <TableCell colSpan={8} className="p-0">
                      <div className={`transition-opacity duration-200 ${combination.isAnalyzing ? 'opacity-50' : ''}`}>
                        <QuerySection
