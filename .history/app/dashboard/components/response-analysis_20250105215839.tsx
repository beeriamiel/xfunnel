  ...
  return [
    {
      ...
      icp: { id: 1, region: "North America", vertical, company_size: "1000+ employees" },
      persona: { id: 1, title, seniority_level: "Senior", department: "Product" },
      status: 'complete'
    },
    {
      id: 2,
                    ...
                    <Card key={query.id} className="p-3 bg-[#f6efff]/50 hover:bg-[#f6efff] transition-colors">
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <Badge variant="outline" className="text-xs border-[#f9a8c9]" children={`${query.icp.region} · ${query.icp.vertical}`} />
                          <Badge variant="outline" className="text-xs border-[#f9a8c9]" children={`${query.persona.title} · ${query.persona.department}`} />
                        </div>
                      </div>
                    </Card>
                  ))}
...
export function ResponseAnalysis({ companyId }: Props) {
  ...
  const setHasCompletedOnboarding = useDashboardStore(state => state.setHasCompletedOnboarding)
  const companyProfile = useDashboardStore(state => state.companyProfile)
  const setCompanyProfile = useDashboardStore(state => state.setCompanyProfile)
  ...
  // Update Badge usage to use proper children prop
  const renderBadge = (content: React.ReactNode) => (
    <div className="text-xs border-[#f9a8c9]">
      <Badge variant="outline">{content}</Badge>
    </div>
  )

  // Update ICP generation to include personas
  function generateICPs(): ICP[] {
    return [
      {
        id: 1,
