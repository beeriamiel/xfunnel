...
export function VerticalView() {
  ...
  useEffect(() => {
    async function loadPersonas() {
      if (!selectedRegion || !selectedVertical || !selectedCompanyId) return
      
      setIsLoading(true)
      try {
        const data = await fetchPersonas(selectedCompanyId, selectedVertical, {
          type: filterType,
    ...
    loadPersonas()
  }, [selectedRegion, selectedVertical, filterType, selectedBatchId, selectedTimePeriod, selectedCompanyId])
                  ...
                  <div className="space-y-2 text-sm text-muted-foreground">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium">{persona.metrics.sentiment.toFixed(1)}</p>
                      </div>
                      <div>
                        <p className="text-xs">Position</p>
                        <p className="font-medium">{persona.metrics.position.toFixed(1)}</p>
                      </div>
                    </div>
                  </div>
                </div>
