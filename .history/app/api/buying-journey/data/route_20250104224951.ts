...
function calculateMetrics(responses: ResponseAnalysis[]): Metrics {
  ...
  const positionResponses = responses.filter(r => 
    ...
    ['solution_comparison', 'user_feedback'].includes(r.buying_journey_stage) &&
  ...
  const earlyStageResponses = responses.filter(r => 
    r.buying_journey_stage && 
    ['problem_exploration', 'solution_education'].includes(r.buying_journey_stage)
  )
  ...
  const evaluationResponses = responses.filter(r => 
    r.buying_journey_stage === 'solution_evaluation' && 
    r.solution_analysis
  )
...
export async function GET(request: Request) {
  try {
    ...
    if (vertical) query = query.eq('icp_vertical', vertical)
