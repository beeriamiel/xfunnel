...
(Note: "chaos" here is not referring to Chaos.com)`;
  ...
  const userPrompt = `Analyze the following text and identify which of these specific companies appear and how they are ranked:
...
Return them in a numbered list based on ranking structure if it exists, or order of appearance if no ranking exists. Only return NO_EXPLICIT_RANKING if none of the listed companies are found.`;
